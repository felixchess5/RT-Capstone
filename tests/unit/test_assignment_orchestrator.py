...
class TestAssignmentOrchestrator:
    ...
    def test_classify_assignment_subject_detection(self, assignment_orchestrator, text, expected_subject):
    ...
    @pytest.mark.parametrize("text,metadata,expected_complexity", [
        ("derivative of x^2", {"class": "Calculus AP"}, AssignmentComplexity.HIGH_SCHOOL),
        ("2 + 2 = ?", {"class": "Grade 2"}, AssignmentComplexity.ELEMENTARY),
        ("integral calculus problem", {"class": "University Math"}, AssignmentComplexity.COLLEGE),
        ("solve for x: x + 1 = 2", {"class": "Middle School Algebra"}, AssignmentComplexity.MIDDLE_SCHOOL)
    ])
    def test_complexity_classification(self, assignment_orchestrator, text, metadata, expected_complexity):
    ...
    def test_classify_assignment_with_metadata(self, assignment_orchestrator):
        ...
        assert classification.complexity in [AssignmentComplexity.HIGH_SCHOOL, AssignmentComplexity.COLLEGE]
    ...
    def test_complexity_determination_advanced(self, assignment_orchestrator):
        ...
        college_text = "Evaluate the limit using L'Hôpital's rule and multivariable calculus"
        college_metadata = {"class": "Advanced Calculus"}

        classification = assignment_orchestrator.classify_assignment(college_text, college_metadata)
        ...
        classification = assignment_orchestrator.classify_assignment(elementary_text, elementary_metadata)
        assert classification.complexity == AssignmentComplexity.ELEMENTARY

    @pytest.mark.asyncio
    async def test_process_assignment_math(self, assignment_orchestrator, sample_math_assignment):
        """Test processing a math assignment."""
        with patch.object(assignment_orchestrator.math_processor, 'grade_math_assignment') as mock_grade:
    ...
    @pytest.mark.asyncio
    async def test_process_assignment_spanish(self, assignment_orchestrator, sample_spanish_assignment):
        """Test processing a Spanish assignment."""
        with patch.object(assignment_orchestrator.spanish_processor, 'grade_spanish_assignment') as mock_grade:
            mock_grade.return_value = {
                'overall_score': 7.5,
    ...
    async def test_process_assignment_error_handling(self, assignment_orchestrator):
        ...
        with patch.object(assignment_orchestrator.math_processor, 'grade_math_assignment') as mock_grade:
            ...
            assert 'Unable to complete specialized analysis.' in result['specialized_feedback']
    ...
    def test_routing_logic(self, assignment_orchestrator):
        ...
        # Test math routing
        math_classification = assignment_orchestrator.classify_assignment("Solve for x: 2x = 10")
        assert math_classification.subject == SubjectType.MATHEMATICS

        # Test Spanish routing
        spanish_classification = assignment_orchestrator.classify_assignment("Hola, ¿cómo estás?")
        assert spanish_classification.subject == SubjectType.SPANISH

        # Test science routing
        science_classification = assignment_orchestrator.classify_assignment("Hypothesis: Plants need sunlight")
    ...
    def test_confidence_levels(self, assignment_orchestrator, confidence_level):
        ...
        with patch.object(assignment_orchestrator, '_calculate_confidence') as mock_confidence:
