...
def test_basic_tools():
    ...
    try:
        ...
        if result.get('grades'):
            ...
            print(f"   Grades: Factuality={grades.get('factuality')}, Relevance={grades.get('relevance')}, Coherence={grades.get('coherence')}, Grammar={grades.get('grammar')}")
    except Exception as e:
        print(f"   Error: {e}")
...
async def test_parallel_processing():
    ...
    try:
        result = await process_assignment_parallel(sample_text, source_text, "Parallel_Test_Student")
        ...
        tools = ['grammar_check', 'plagiarism_check', 'relevance_check', 'grading', 'summary']
        for tool in tools:
...
def test_environment():
    ...
    print(f"Assignments folder: {ASSIGNMENTS_FOLDER} ({'exists' if os.path.exists(ASSIGNMENTS_FOLDER) else 'missing'})")
    print(f"Reports folder: {PLAGIARISM_REPORTS_FOLDER} ({'exists' if os.path.exists(PLAGIARISM_REPORTS_FOLDER) else 'missing'})")
