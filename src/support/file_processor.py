...
class FileProcessingResult:
    ...
    def __init__(
        ...
        metadata: Dict = None,
        error: str = "",
        rejection_reason: Optional[FileRejectionReason] = None,
    ):
...
class FileProcessor:
    ...
    def detect_file_format(self, file_path: str) -> Tuple[FileFormat, str]:
        ...
        try:
            ...
            if format_by_ext != FileFormat.UNKNOWN:
                if format_by_signature == format_by_ext or format_by_signature in (
                    FileFormat.UNKNOWN,
                    FileFormat.TXT,
                ):
                    ...
                    return (
                        format_by_signature,
                        f"Signature override extension ({format_by_signature.value})",
                    )
    ...
    def extract_text_details(self, file_path: str) -> FileProcessingResult:
        return (
            super().extract_text_content(file_path)
            if hasattr(super(), "extract_text_content")
            else FileProcessingResult(False, error="Not available")
        )
    ...
    def _extract_pdf_file(self, file_path: str) -> FileProcessingResult:
        ...
        if not content.strip():
            ...
            try:
                ...
                from ocr_processor import extract_text_from_scanned_pdf, is_scanned_pdf
                ...
                if is_scanned_pdf(file_path):
                    ...
                    if content.strip():
                        ...
                        detected_languages = (
                            [lang_result.fallback_language]
                            if lang_result.is_supported
                            else None
                        )

                    ocr_result = extract_text_from_scanned_pdf(
                        file_path, enhanced=True, language_codes=detected_languages
                    )
                    ...
                    if ocr_result.success and ocr_result.text.strip():
                        ...
                        return FileProcessingResult(
                            success=True, content=ocr_result.text, metadata=metadata
                        )
                    else:
                        logger.warning(
                            f"OCR failed for {file_path}: {ocr_result.error}"
                        )
    ...
    def _extract_markdown_file(self, file_path: str) -> FileProcessingResult:
        ...
        try:
            ...
            return FileProcessingResult(
                success=True, content=text_content, metadata=metadata
            )
    ...
    def detect_content_type(self, text: str) -> str:
        ...
        if any(
            k in t
            for k in ["solve", "equation", "derivative", "integral", " x ", " y="]
        ):
            return "math"
...
class OCRProcessor:
    def extract_text_from_image(self, file_path: str) -> str:
        try:
            ...
            result = extract_text_from_image_file(
                file_path, enhanced=True, language_codes=["en", "es"]
            )
    ...
    def _extract_image_file(self, file_path: str) -> FileProcessingResult:
        ...
        try:
            ...
            ocr_result = extract_text_from_image_file(
                file_path, enhanced=True, language_codes=common_languages
            )
    ...
    def get_rejection_message(
        self, rejection_reason: FileRejectionReason, file_path: str = ""
    ) -> str:
        """
        ...
        """
        messages = {
            FileRejectionReason.UNSUPPORTED_FORMAT: f"File format not supported. Please use TXT, PDF, DOCX, DOC, or MD files. "
            f"Detected format could not be determined for: {os.path.basename(file_path)}",
            FileRejectionReason.CORRUPTED_FILE: f"File appears to be corrupted or damaged. Please check the file integrity and try again. "
            f"File: {os.path.basename(file_path)}",
            FileRejectionReason.EMPTY_CONTENT: f"File contains no readable content or content is too short (minimum {self.MIN_CONTENT_LENGTH} characters required). "
            f"Please ensure the file has substantive content. File: {os.path.basename(file_path)}",
            FileRejectionReason.ENCODING_ERROR: f"Unable to read file due to encoding issues. Please ensure the file is saved with UTF-8 encoding or is not corrupted. "
            f"File: {os.path.basename(file_path)}",
            FileRejectionReason.PERMISSION_ERROR: f"Cannot access file due to permission restrictions or file does not exist. "
            f"Please check file permissions and path. File: {os.path.basename(file_path)}",
            FileRejectionReason.FILE_TOO_LARGE: f"File size exceeds maximum limit of {self.MAX_FILE_SIZE // (1024*1024)}MB. "
            f"Please reduce file size or split into smaller files. File: {os.path.basename(file_path)}",
            FileRejectionReason.MISSING_METADATA: f"Required metadata is missing from the file. Please ensure the file contains proper student information headers. "
            f"File: {os.path.basename(file_path)}",
            FileRejectionReason.INVALID_STRUCTURE: f"File structure is invalid or does not meet assignment requirements. "
            f"Please check the file format and content structure. File: {os.path.basename(file_path)}",
        }

        return messages.get(
            rejection_reason,
            f"File rejected for unknown reason. File: {os.path.basename(file_path)}",
        )
...
if __name__ == "__main__":
    ...
    test_files = ["test.txt", "document.pdf", "assignment.docx", "readme.md"]
