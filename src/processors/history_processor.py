...
class HistoryProcessor:
    ...
    def analyze_history_assignment(self, text: str) -> HistoryAnalysis:
        ...
        chronological_accuracy = self.assess_chronological_accuracy(
            text, dates_identified
        )
    ...
    async def grade_history_assignment(
        ...
        grading_prompt = self._create_history_grading_prompt(
        ...
        try:
            if llm_manager:
                response = llm_manager.invoke_with_fallback(
                    grading_prompt, use_case="history_analysis"
                )
    ...
    def _create_history_grading_prompt(
        self, text: str, analysis: HistoryAnalysis, source_text: str = None
    ) -> str:
    ...
    def _calculate_history_scores(self, analysis: HistoryAnalysis) -> Dict[str, float]:
        ...
        scores["historical_accuracy"] = min(
            analysis.historical_vocabulary_score + 1.0, 10.0
        )
    ...
    def _generate_history_insights(self, analysis: HistoryAnalysis) -> List[str]:
        ...
        insights.append(
            f"Assignment type: {analysis.assignment_type.value.replace('_', ' ')}"
        )
        insights.append(
            f"Regional focus: {analysis.region_focus.value.replace('_', ' ')}"
        )

        if analysis.dates_identified:
            insights.append(
                f"References {len(analysis.dates_identified)} specific dates/time periods"
            )

        if analysis.historical_figures:
            insights.append(
                f"Mentions {len(analysis.historical_figures)} historical figures"
            )

        if analysis.events_mentioned:
            insights.append(
                f"Discusses {len(analysis.events_mentioned)} historical events"
            )
        ...
        if analysis.sources_cited:
            ...
            insights.append(
                "No sources cited - consider adding primary/secondary sources"
            )
        ...
        if analysis.bias_awareness_score > 7.0:
            ...
            insights.append(
                "Limited awareness of historical perspective and potential bias"
            )

        return insights

    def _generate_history_recommendations(
        self, analysis: HistoryAnalysis, scores: Dict[str, float]
    ) -> List[str]:
        ...
        if scores.get("chronological_understanding", 0) < 6.0:
            if len(analysis.dates_identified) == 0:
                recommendations.append(
                    "Include specific dates and time periods to establish chronology"
                )
            else:
                recommendations.append(
                    "Improve chronological organization and sequencing"
                )
        ...
        if scores.get("source_analysis", 0) < 6.0:
            if len(analysis.sources_cited) == 0:
                recommendations.append(
                    "Include primary and secondary sources to support arguments"
                )
            else:
                recommendations.append(
                    "Analyze sources more critically for bias and reliability"
                )

        # Contextual awareness
        if scores.get("contextual_awareness", 0) < 6.0:
            recommendations.append(
                f"Provide more context about {analysis.period.value.replace('_', ' ')} period conditions"
            )

        # Argument development
        if scores.get("argument_development", 0) < 6.0:
            recommendations.append(
                "Strengthen argument structure with clear thesis and supporting evidence"
            )
